...
def evaluate_all_videos():
    ...
    for filename in os.listdir(correctly_worn_dir):
        if filename.endswith('.mp4'):
            ...
            try:
                ...
                print(f"✅ {filename}: {'Correct' if prediction == 1 else 'Wrong'} (conf: {confidence:.3f})")
            except Exception as e:
                print(f"❌ Error processing {filename}: {str(e)}")
    
    print("\nEvaluating incorrectly worn videos...")
    for filename in os.listdir(incorrectly_worn_dir):
        if filename.endswith('.mp4'):
            ...
            try:
                ...
                print(f"✅ {filename}: {'Correct' if prediction == 0 else 'Wrong'} (conf: {confidence:.3f})")
            except Exception as e:
                print(f"❌ Error processing {filename}: {str(e)}")
